...
function CartPage() {
  ...
  const [PreviousItems, setPreviousItem] = useState([allCart.length]);

  useEffect(() => {
    if (allCart.length < PreviousItems) {
      setPreviousItem(allCart.length);
    } else if (allCart.length > PreviousItems) {
      setPreviousItem(allCart.length);
    }
  }, [allCart.length, PreviousItems]);

  const total =
    allCart.length === 0
      ? 0
      : allCart
          ?.map((item) => (item.Sum ? item.price * item.cartQuantity : 0))
          .reduce((itemPrice, accPrice) => accPrice + itemPrice);

  const checkoutHandler = () => {
    const item = allCart.filter((item) => item.Sum).map((item) => item.id);
    dispatch(batchRemove({ id: item }));
  };
  return (
    <div className="layout max-w-screenxl my-8 mx-auto md:my-12">
      ...
      <p>{`have ${allCart.length} items in cart`}</p>
